...
byte *iscsi_alloc_dsbuf(struct iscsi_conn *conn, uint32 dslen) {
    ...
    if (ds == NULL) {
        log_err("Unable to allocate memory (dslen="
                        U32_FMT
                        ").\n", dslen);
        return NULL;
    }
    ...
    log_dbg3("Allocated memory (conn->dslen_total="
                     U32_FMT
                     ").\n",
             conn->dslen_total);
...
int iscsi_free_dsbuf(struct iscsi_conn *conn, byte *ds, uint32 dslen) {
    ...
    log_dbg3("Free memory (conn->dslen_total="
                     U32_FMT
                     ").\n",
             conn->dslen_total);
...
int iscsi_add_pdu_to_task(struct iscsi_conn *conn, struct iscsi_task *task, struct iscsi_pdu *pdu) {
    ...
    log_dbg3("-----\n");
    log_dbg3("Add PDU (pdu->{opcode=0x%02X, itt=0x%08lX}, task->list_pdu.len="
                     U32_FMT
                     ").\n",
...
struct iscsi_task *iscsi_create_task(struct iscsi_conn *conn, uint32 itt) {
    ...
    log_dbg3("Created iSCSI task (itt=0x%08lX, conn->list_task.len="
                     U32_FMT
                     ").\n",
...
int iscsi_remove_task(struct iscsi_conn *conn, struct iscsi_task *task) {
    ...
    log_dbg3("Remove iSCSI task (task->list_pdu.len="
                     U32_FMT
                     ", conn->list_task.len="
                     U32_FMT
                     ", conn->pdus="
    ...
    ASSERT(task->list_pdu.len == 0, "task->list_pdu.len("
            U32_FMT
            ") > 0\n", task->list_pdu.len);
    ...
    log_dbg3("Remove iSCSI task (conn->task_cnt="
                     U32_FMT
                     ", conn->pdus="
                     U32_FMT
                     ").\n",
...
struct iscsi_pdu *iscsi_create_pdu(struct iscsi_conn *conn) {
    ...
    log_dbg3("Created iSCSI PDU (conn->pdus="
                     U32_FMT
                     ").\n", conn->pdus);
...
void iscsi_remove_pdu(struct iscsi_conn *conn, struct iscsi_pdu *pdu) {
    ...
    log_dbg3("Free an iSCSI PDU (pdu->{opcode=0x%02X, itt=0x%08lX}, conn->pdus="
                     U32_FMT
                     ").\n",
