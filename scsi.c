...
static int create_and_send_scsidata_in_by_page(
    ...
    do_each_list_elem (struct page_buffer *, list_page, page, listelem) {
        ...
        while (1) {
            ...
            if (page_totallen > pdu_req->expdtlen &&
                ...
                pdu_rsp->Obit = 1;
                overflow = 1;
                log_dbg3("Residual overflow occured (itt="
                                 U32_FMT
                                 "(0x%08lX), datasn="
                                 U32_FMT
                                 "(0x%08lX), "
    ...
    if (page_totallen < pdu_req->expdtlen) {
        ...
        log_dbg3("Residual underflow occured (itt="
                         U32_FMT
                         "(0x%08lX), datasn="
                         U32_FMT
                         "(0x%08lX), "
...
static int create_and_send_scsidata_in(
    ...
    log_dbg3("pdu_req->expdtlen="
                     U32_FMT
                     "(0x%08lX), pdu_rsp->dslen="
                     U32_FMT
                     "(0x%08lX)\n",
             pdu_req->expdtlen, pdu_req->expdtlen,
             pdu_rsp->dslen, pdu_rsp->dslen);
...
int scsi_dump_cdb(struct iscsi_conn *conn, struct scsi_cmd *cmd) {
    ...
    switch (cmd->opcode) {
        ...
        case SCSI_OP_REPORT_LUNS:
            ...
            log_dbg2("  select report = "
                             U32_FMT
                             "\n", cmd->select_report);
            log_dbg2("  allocation length = "
                             U32_FMT
...
static int enqueue_volcmd(
    ...
    if (volcmd == NULL) {
        log_err("Unable to allocate volume command ("
                        U32_FMT
                        ").\n",
