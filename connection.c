...
static int iscsi_add_pdu_to_iovec(struct iscsi_conn *conn, struct iscsi_pdu *pdu) {
    ...
    ASSERT((conn->iov_tx.len == 0), "conn->iov_tx.len("
            U32_FMT
            ") > 0\n", conn->iov_tx.len);
...
int iscsi_enqueue_and_tx_pdu(
    ...
    log_dbg3("Add PDU to TX queue (opcode=0x%02X, ITT=0x%08lX, conn->list_pdu_tx.len="
                     U32_FMT
                     ").\n",
...
//	conn->stage = ISCSI_STAGE_FINISH;
    ...
    if (!list_is_empty(&(conn->list_pdu_tx))) {
        ...
        pdu = (struct iscsi_pdu *) list_ref_head_elem(&(conn->list_pdu_tx));
        iscsi_add_pdu_to_iovec(conn, pdu);
        log_dbg3("Continue to send next PDU (conn->list_pdu_tx.len="
                         U32_FMT
                         ").\n",
    ...
    ASSERT(conn->list_pdu_tx.len == 0, "conn->list_pdu_tx.len("
            U32_FMT
            ") > 0\n", conn->list_pdu_tx.len);
    if (ev->events & EPOLLOUT) {
        // stop event-polling
...
static int iscsi_exec_rx(struct iscsi_conn *conn) {
    ...
    switch (conn->state_rx) {
        ...
        case SOCKIO_BHS_RXTX:
            ...
            log_dbg3("Received PDU's BHS (opcode=0x%02X, ITT=0x%08X, DSlen="
                             U32_FMT
                             ").\n",
        ...
        case SOCKIO_DS_INIT:
            ...
            if (pdu->dslen > 0) {
                log_dbg3("pdu->task->list_page.len = "
                                 U32_FMT
                ...
                if (list_is_empty(&(pdu->task->list_page))) {
                    ...
                    pdu->dsvec_cnt = idx;
                    log_dbg3("BEFORE: pdu->task->page_filled="
                                     U32_FMT
                                     "\n", pdu->task->page_filled);
                    pdu->task->page_filled += pdu->dslen;
                    log_dbg3("AFTER:  pdu->task->page_filled="
                                     U32_FMT
                                     "\n", pdu->task->page_filled);
                }
...
int exec_scsidata_out(struct iscsi_conn *conn, struct iscsi_pdu *pdu) {
    ...
    log_dbg3("task->page_filled("
                     U32_FMT
                     "(0x%08lX)), task->page_totallen("
                     U32_FMT
                     "(0x%08lX))\n",
             task->page_filled, task->page_filled,
