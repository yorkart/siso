...
int vol_alloc_buf(
    ...
    log_dbg3("lba=%llu(0x%016llX), trans_len="
                     U32_FMT
                     "(0x%08lX)\n",
...
int vol_free_buf(
    ...
    ASSERT((list_page->len == 0), "list_page->len("
            U32_FMT
            ") > 0\n", list_page->len);
    ...
    LOCK_LIST_CMD(vol);
    {
        ...
        switch (status) {
            case VOLCMD_STATUS_DOING:
                ...
                break;
            case VOLCMD_STATUS_CANCELED:
                log_wrn("IO is canceled (SCSI opcode=0x%02X, LBA=%llu, trans_len="
                                U32_FMT
                                ")\n",
